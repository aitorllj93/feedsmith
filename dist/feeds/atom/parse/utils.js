import { createNamespaceGetter, isObject, isPresent, parseArrayOf, parseNumber, parseSingularOf, parseString, parseTextString, retrieveText, trimObject, } from '../../../common/utils.js';
...
import { retrieveItem as retrieveThrItem, retrieveLink as retrieveThrLink, } from '../../../namespaces/thr/utils.js';
export const parseLink = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const link = {
...
export const retrievePersonUri = (value, options) => {
    if (!isObject(value)) {
        return;
    }
...
export const parsePerson = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const get = createNamespaceGetter(value, options?.prefix);
...
export const retrieveGeneratorUri = (value) => {
    if (!isObject(value)) {
        return;
    }
...
export const parseGenerator = (value) => {
    const generator = {
...
export const parseSource = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const get = createNamespaceGetter(value, options?.prefix);
...
export const retrievePublished = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const get = createNamespaceGetter(value, options?.prefix);
...
export const retrieveUpdated = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const get = createNamespaceGetter(value, options?.prefix);
...
export const retrieveSubtitle = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const get = createNamespaceGetter(value, options?.prefix);
...
export const parseEntry = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const get = createNamespaceGetter(value, options?.prefix);
...
export const parseFeed = (value, options) => {
    if (!isObject(value)) {
        return;
    }
    const get = createNamespaceGetter(value, options?.prefix);
...
export const retrieveFeed = (value) => {
    ...
    const namespaced = parseSingularOf(value?.['atom:feed'], (value) => parseFeed(value, { prefix: 'atom:' }));
