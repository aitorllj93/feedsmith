import { isNonEmptyString, isObject, isPresent, parseArrayOf, parseNumber, parseSingularOf, parseTextNumber, parseTextString, retrieveText, trimObject, } from '../../common/utils.js';
export const parseLatLngPairs = (value, pairsCount) => {
    ...
    if ((pairsCount?.min && actualPairCount < pairsCount?.min) ||
...
export const parsePolygon = (value) => {
    const points = parseLatLngPairs(retrieveText(value), { min: 4 });
    if (isPresent(points)) {
        return { points };
    }
};
export const parseBox = (value) => {
